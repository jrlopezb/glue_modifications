...
R<-RunNumber;#Get the what parameter set will be used to change the genotype file.
...
} else
{
  ...
  eval(parse(text=paste('ECOFilePath="',GD,'/SCCAN045.ECO"',sep = '')));
  ReadLine<-readLines(ECOFilePath, n=-1)
  ECOFile<-as.character(ReadLine);
  #Get the ECO file from the Genotype directory.
  
  eval(parse(text=paste("NewECOFilePath='",OD,"/SCCAN045.ECO'",sep = '')));
